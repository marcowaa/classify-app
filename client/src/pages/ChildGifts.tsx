...
export const ChildGifts = (): JSX.Element => {
  ...
  const availableGifts = giftsList.filter((gift: any) => currentPoints >= Number(gift.pointsPrice || 0)).length;
  const closestGift = giftsList
    .filter((gift: any) => currentPoints < Number(gift.pointsPrice || 0))
    .sort((a: any, b: any) => (Number(a.pointsPrice || 0) - currentPoints) - (Number(b.pointsPrice || 0) - currentPoints))[0];
  const pointsToClosestGift = closestGift ? Math.max(0, Number(closestGift.pointsPrice || 0) - currentPoints) : 0;
  ...
  return (
    <div className={`min-h-screen pb-24 ${isDark ? "bg-gradient-to-br from-purple-900 to-indigo-900" : "bg-gradient-to-br from-purple-400 to-pink-400"} p-4 md:p-8`} dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="sticky top-0 z-30 py-2 mb-6 backdrop-blur-sm">
          <div className={`rounded-2xl px-4 py-3 ${isDark ? "bg-slate-900/70 border border-slate-700/60" : "bg-white/70 border border-white/70"} shadow-lg` }>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="px-4 py-2 bg-fuchsia-500 hover:bg-fuchsia-600 text-white font-bold rounded-lg transition-all"
              >
                🛍️ {t("childNav.store", t("store.title", "المتجر"))}
              </button>
              <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
          {[
            { title: t("child.myGifts"), value: giftsList.length, color: "from-fuchsia-500 to-pink-500", icon: "🎁" },
            { title: t("child.available"), value: availableGifts, color: "from-emerald-500 to-green-500", icon: "✅" },
            { title: t("childProfile.about.totalPoints"), value: currentPoints, color: "from-yellow-500 to-amber-500", icon: "⭐" },
            { title: t("child.needPoints"), value: pointsToClosestGift, color: "from-indigo-500 to-blue-500", icon: "🎯" },
          ].map((card) => (
            <div key={card.title} className={`rounded-2xl p-3 text-white bg-gradient-to-br ${card.color} shadow-lg`}>
              <p className="text-lg">{card.icon}</p>
              <p className="text-2xl font-black leading-tight">{card.value}</p>
              <p className="text-xs opacity-95 truncate">{card.title}</p>
            </div>
          ))}
        </div>
