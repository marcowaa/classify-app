...
const LAST_PERMISSION_PROMPT_KEY = "child_push_last_permission_prompt_at";
    ...
    const run = async () => {
      try {
        ...
        if (permission !== "granted") {
          emitNotificationPermissionRequired("child", permission === "denied" ? "denied" : "default");
          scheduleRetry(permission === "denied" ? DENIED_RETRY_MS : DEFAULT_RETRY_MS);
          return;
        }

        markNotificationPermissionGranted("child");

        const keyRes = await fetch("/api/child/push-public-key", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!keyRes.ok) {
          emitNotificationPermissionRequired("child", "subscription_error");
          scheduleRetry(DEFAULT_RETRY_MS);
          return;
        }
        ...
        const saveRes = await fetch("/api/child/push-subscriptions", {
          method: "POST",
          headers: {
        ...
        emitNotificationPermissionRequired("child", "registration_error");
        scheduleRetry(DEFAULT_RETRY_MS);
      }
    };
