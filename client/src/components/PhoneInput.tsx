...
export const PhoneInput: React.FC<PhoneInputProps> = ({
  ...
  return (
    <div className="space-y-1.5 min-w-0">
      {showLabel && (
        <label className="block text-xs sm:text-sm leading-tight font-semibold text-gray-700 dark:text-gray-300 flex items-center gap-1.5">
          <Phone className="w-3.5 h-3.5" />
          <span className="break-words">رقم الهاتف / Phone Number</span>
        </label>
      )}

      <div className="flex gap-0 relative min-w-0 w-full" ref={dropdownRef}>
        ...
        <button
          ...
          disabled={disabled}
          className={`
            flex items-center gap-1 px-2 min-[390px]:px-2.5 min-h-11 py-3 rounded-r-xl border-2 border-l-0
            transition-all duration-200 min-w-[84px] min-[360px]:min-w-[92px] sm:min-w-[110px] justify-between shrink-0
            ${isOpen
          ...
          `}
        >
          <span className="text-base min-[390px]:text-lg leading-none">{selectedCountry?.flag}</span>
          <span className="text-[11px] min-[360px]:text-xs min-[390px]:text-sm font-bold text-gray-800 dark:text-gray-100 tabular-nums">
        ...
        <input
          ...
          dir="ltr"
          className={`
            auth-mobile-input flex-1 min-h-11 px-3 min-[390px]:px-4 py-3 rounded-l-xl border-2 text-base font-medium tracking-wide
        ...
        {isOpen && (
          <div
            className="
              absolute top-full right-0 left-0 mt-1.5 z-[60]
            ...
            <div className="sticky top-0 bg-white dark:bg-gray-800 p-2.5 border-b border-gray-100 dark:border-gray-700">
              <div className="relative">
                ...
                <input
                  ...
                  className="
                    auth-mobile-input w-full pr-9 pl-8 py-2.5 text-base rounded-lg
                ...
                filtered.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`tabular-nums font-bold text-xs min-w-[50px] text-left ${isSelected ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"
                        }`}>
                        {c.dialCode}
                      </span>
                      {isSelected && (
