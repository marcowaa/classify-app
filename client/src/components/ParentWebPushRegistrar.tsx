...
const LAST_PERMISSION_PROMPT_KEY = "parent_push_last_permission_prompt_at";
...
function getOrCreateDeviceId(): string {
  const key = "parent_push_device_id";
...
export function ParentWebPushRegistrar() {
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (permission === "default" && canPromptAgain(DEFAULT_RETRY_MS)) {
          ...
          // Browsers may not always re-show the native prompt after denial,
          // but this keeps retry behavior aligned with product requirements.
          markPromptAttempt();
          permission = await Notification.requestPermission();
        }

        if (permission !== "granted") {
          emitNotificationPermissionRequired("parent", permission === "denied" ? "denied" : "default");
          scheduleRetry(permission === "denied" ? DENIED_RETRY_MS : DEFAULT_RETRY_MS);
          return;
        }

        markNotificationPermissionGranted("parent");

        const keyRes = await fetch("/api/parent/push-public-key", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!keyRes.ok) {
          emitNotificationPermissionRequired("parent", "subscription_error");
          scheduleRetry(DEFAULT_RETRY_MS);
          return;
        }
        ...
        const saveRes = await fetch("/api/parent/push-subscriptions", {
          method: "POST",
          headers: {
        ...
        console.error("Parent web push registration error:", error);
        emitNotificationPermissionRequired("parent", "registration_error");
        scheduleRetry(DEFAULT_RETRY_MS);
      }
    };
...
export default ParentWebPushRegistrar;
