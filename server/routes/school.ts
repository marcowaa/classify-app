...
async function generateEntityReferralCode(
  prefix: string,
  length: number,
...
export async function registerSchoolRoutes(app: Express) {
  ...
  app.put("/api/school/uploads/proxy", schoolMiddleware, uploadProxyLimiter, async (req: SchoolRequest, res) => {
    try {
  ...
  app.post("/api/school/teachers", schoolMiddleware, async (req: SchoolRequest, res) => {
    try {
      ...
      const referralCode = await generateEntityReferralCode(
        "TCH",
        8,
        async (candidate: string) => {
          const exists = await db.select({ id: schoolTeachers.id }).from(schoolTeachers)
  ...
  app.post("/api/store/schools/:id/referral-click", optionalAuthMiddleware, async (req: any, res) => {
    try {
      const { id } = req.params;
      const { referralCode } = req.body || {};

      const school = await db.select().from(schools).where(and(eq(schools.id, id), eq(schools.isActive, true))).limit(1);
      if (!school[0]) {
        return res.status(404).json({ success: false, error: "NOT_FOUND", message: "School not found" });
      }

      const normalizedCode = String(referralCode || school[0].referralCode || "").trim().toUpperCase();
      if (!normalizedCode) {
      ...
      await logSchoolActivity(id, "referral_click", 1, {
        referralCode: normalizedCode,
        sourceType: req.authUser?.type || "guest",
      });

      return res.json({ success: true });
    } catch (error: any) {
      console.error("Record school referral click error:", error);
  ...
  app.post("/api/school/enrollments/bulk-action", schoolMiddleware, async (req: any, res) => {
    try {
      ...
      if (action === "approve") {
        ...
        const referralSettings = await db.select().from(schoolReferralSettings).limit(1);
        const schoolReferralPoints = referralSettings[0]?.pointsPerStudentJoin || 10;
        ...
        for (const enrollment of enrollmentRows) {
          ...
          const referralMatch = await db.select().from(schoolReferrals)
            .where(
              and(
                eq(schoolReferrals.schoolId, schoolId),
                eq(schoolReferrals.referredParentId, enrollment.parentId),
                eq(schoolReferrals.status, "clicked")
              )
            )
          ...
          if (referralMatch[0]) {
            await db.update(schoolReferrals)
              .set({
                status: "enrolled",
                referredChildId: enrollment.childId,
                pointsAwarded: schoolReferralPoints,
                convertedAt: new Date(),
              })
              .where(eq(schoolReferrals.id, referralMatch[0].id));

            await logSchoolActivity(schoolId, "referral_enrolled", schoolReferralPoints, {
              referralId: referralMatch[0].id,
              referralCode: referralMatch[0].referralCode,
